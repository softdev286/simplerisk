...
//define('VERSION_NAME', 'complianceforge_extra_version');

global $complianceforge_updates;
$complianceforge_updates = array(
);

/****************************************************
 * FUNCTION: UPGRADE COMPLIANCEFORGE EXTRA DATABASE *
 ****************************************************/
function upgrade_complianceforge_extra_database()
{
    global $complianceforge_updates;

    $version_name = 'complianceforge_extra_version';
    ...
    if (array_key_exists($db_version, $complianceforge_updates))
    {
        // Get the function to upgrade to the next version
        $function = $complianceforge_updates[$db_version];
